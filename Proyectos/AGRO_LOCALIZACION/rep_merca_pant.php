...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $sql = "SELECT id_agro, nombre, direccion, coordenadas FROM agromercados";
        ...
        <head>
            ...
            <title>Reporte de Agromercados</title>
            <style>
                body {
        ...
        <body>
            <h1>Reporte de Todos los Agromercados</h1>
            <table>
                <tr>
                    <th>ID Agromercado</th>
                    <th>Nombre </th>
                    <th>Direccion</th>
                    <th>coordenadas</th>
                </tr>';
        ...
        if ($stmt->rowCount() > 0) {
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $html .= '
                <tr>
                    <td>' . htmlspecialchars($row['id_agro']) . '</td>
                    <td>' . htmlspecialchars($row['nombre']) . '</td>
                    <td>' . htmlspecialchars($row['direccion']) . '</td>
                    <td>' . htmlspecialchars($row['coordenadas']) . '</td>
                </tr>';
            }
        } else {
